"""
Order fulfillment pipeline: polls Wix for unfulfilled orders, submits to Zendrop,
then updates Wix with tracking numbers when Zendrop confirms shipment.
"""
import json
import os
import sqlite3
...
def _map_wix_order_to_zendrop(order: dict) -> dict:
    """Convert a Wix order dict into a Zendrop order submission payload."""
...
def run_fulfillment_cycle(catalog_version: str = "v3") -> dict:
    """
    Fetch unfulfilled Wix orders and submit each to Zendrop.
    Skips orders already recorded in the DB.
    """
    init_db()
...
def check_pending_fulfillments(catalog_version: str = "v3") -> dict:
    """
    For all submitted-but-not-shipped orders in DB: check Zendrop for tracking,
    then update Wix when tracking is available.
    """
    init_db()

    with _db() as conn:
        rows = conn.execute(
            "SELECT wix_order_id, zendrop_order_id FROM fulfillments WHERE status = 'submitted'"
        ).fetchall()
        ...
        try:
            ...
            if tracking:
                ...
                with _db() as conn:
                    conn.execute(
                        """UPDATE fulfillments
                           SET status='fulfilled', tracking_number=?, carrier=?, updated_at=?
                           WHERE wix_order_id=?""",
