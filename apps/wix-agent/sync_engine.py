"""
Sync engine: joins Wix inventory with Zendrop stock data.
All writes are logged before/after. Nothing is changed without explicit confirmation.
"""
import json
import os
...
def audit_inventory(catalog_version: str = "v3") -> dict:
    """
    Read-only audit. Joins Wix products + inventory items, then checks Zendrop.
    Returns a structured report — nothing is written to Wix.
    """
    ...
    # Group inventory items by productId
    ...
    print(f"Fetched {len(zendrop_products)} Zendrop products. Building audit report...")
    ...
    for product_id, product in product_map.items():
        ...
        # Determine current Wix status from inventory items
        ...
        wix_status = "IN_STOCK" if all(s == "IN_STOCK" for s in wix_statuses) and wix_statuses else (wix_statuses[0] if wix_statuses else "NO_INVENTORY_ITEM")
        wix_qty = sum(wix_quantities)

        # Match to Zendrop by name (fuzzy)
...
def fix_inventory(
    ...
    """
    Apply inventory fixes for confirmed product IDs only.
    wix_items: the 'broken' list from audit_inventory().
    confirmed_product_ids: product IDs the user has confirmed to fix.
    dry_run=True returns what would change without writing anything.
    """
    changes = []
    skipped = []
...
def full_sync(catalog_version: str = "v3") -> dict:
    """
    Automated sync: audits and auto-fixes items where Zendrop has stock but Wix shows out.
    Also hides products that have been OUT_OF_STOCK in Zendrop (no inventory items in stock).
    Intended for scheduled runs (n8n every 30 min).
    """
    ...
    no_match = report["no_zendrop_match"]
    hidden = []
