...
def detect_catalog_version() -> str:
    """Returns 'v3' or 'v1' depending on which catalog the site uses."""
    try:
        r = httpx.get(
            f"{BASE_V3}/provision/version",
            headers=_headers(),
            timeout=TIMEOUT,
        )
...
def get_all_products(catalog_version: str = "v3") -> list[dict]:
    """Cursor-paginate through all products. Returns list of product dicts."""
    if catalog_version == "v3":
        url = f"{BASE_V3}/products/query"
    else:
    ...
    while True:
        if first_page:
            ...
            # On follow-up pages: ONLY pass the cursor, no filter/sort
...
def get_all_inventory_items(catalog_version: str = "v3") -> list[dict]:
    """Cursor-paginate through all inventory items (up to 1000/page for v3)."""
    if catalog_version == "v3":
    ...
    while True:
        ...
        page_items = data.get("inventoryItems", [])
...
def update_inventory_item_v3(item_id: str, in_stock: bool, quantity: Optional[int] = None, track_quantity: bool = True) -> dict:
    """PATCH a v3 inventory item to set stock status."""
...
def update_inventory_item_v1(product_id: str, in_stock: bool, quantity: Optional[int] = None) -> dict:
    """PATCH a v1 inventory item to set stock status."""
    url = f"https://www.wixapis.com/stores/v2/inventoryItems/product/{product_id}"
    body = {
        "inventoryItem": {
            "trackQuantity": quantity is not None,
        }
    }
...
def set_product_visibility(product_id: str, visible: bool, catalog_version: str = "v3") -> dict:
    """Show or hide a product on the storefront."""
    if catalog_version == "v3":
        url = f"{BASE_V3}/products/{product_id}"
        body = {"product": {"visible": visible}}
    else:
...
def get_orders(fulfillment_status: str = "NOT_FULFILLED", limit: int = 100) -> list[dict]:
    """Return unfulfilled orders from the Wix eCommerce Orders API."""
...
def fulfill_order(order_id: str, tracking_number: str, shipping_provider: str = "OTHER") -> dict:
    """Mark a Wix order as fulfilled and add tracking."""
