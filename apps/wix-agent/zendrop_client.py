...
def get_all_products(page: int = 1, per_page: int = 100) -> list[dict]:
    ...
    current_page = page
    while True:
        r = httpx.get(url, headers=_headers(), params={"page": current_page, "per_page": per_page}, timeout=TIMEOUT)
        r.raise_for_status()
        data = r.json()
...
def get_product_stock(zendrop_product_id: str) -> dict:
    ...
    if not variants:
        ...
        if total_qty > 0:
            any_in_stock = True
    return {"zendrop_product_id": zendrop_product_id, "in_stock": any_in_stock, "quantity": total_qty, "variants": variants}
...
def search_product_by_name(name: str) -> list[dict]:
    ...
    try:
        r = httpx.get(url, headers=_headers(), params={"q": name}, timeout=TIMEOUT)
