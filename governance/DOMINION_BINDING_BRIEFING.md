...
- **Nemotron is FROZEN** (VM build effort only): not verified built on VM, no VM repo found. Recovery order: Command Vault search → hash evidence → contract comparison → KEEP/FIX/BUILD decision → lifecycle gates (SIMULATION → SHADOW → CANARY → LIMITED_PRODUCTION). Founder approval required to lift FROZEN on VM. Do not write it from scratch before the recovery search.
...
---
*Filed: 2026-08-13 | Document 01 of 11*
