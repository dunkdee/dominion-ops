...
# Work Order 001A — Approval Gate F v2
...
printf '%s\n' '=== WORK ORDER 001A GATE F GUARDED WINDOWS EXTRACTION V2 ==='
printf 'local_evidence_dir=%s\n' "$EVIDENCE_DIR"
printf 'staging_path=%s\n' "$STAGING"

for required_command in awk basename chmod find grep mktemp powershell.exe sha256sum stat tr wc; do
...
export DOMINION_PYTHON_SCRIPT="$PYTHON_SCRIPT"
export DOMINION_ARCHIVE="$ARCHIVE"
export DOMINION_EXTRACTED="$EXTRACTED"
export DOMINION_EXTRACTION_REPORT="$EXTRACTION_REPORT"
...
foreach ($candidate in $candidates) {
  $probe = @($candidate.Prefix) + @("-c", "import os,sys; raise SystemExit(0 if os.name == 'nt' and sys.version_info >= (3,8) else 1)")
