...
# Work Order 001A — Approval Gate F v4
...
#
# Changes from v3:
#   1. MAX_WINDOWS_TARGET_LENGTH raised from 240 to 259 — the blocked path was
#      241 characters; this preserves a guarded legacy Windows ceiling without
#      changing registry policy or enabling Windows long paths.
#   2. max_target_path_length tracked across the complete pre-extraction scan
#      and written to the extraction report, the Gate F READY marker, and the
#      final Gate F output.
#   3. After Python returns, max_target_path_length is parsed from the report,
#      validated as numeric, and confirmed to be no greater than 259 before
...
printf '%s\n' '=== WORK ORDER 001A GATE F GUARDED WINDOWS EXTRACTION V4 ==='
...
# Parse max_target_path_length from the extraction report and validate it.
max_target_path_length=$(awk -F= '$1 == "max_target_path_length" { print $2 }' "$EXTRACTION_REPORT")
case "$max_target_path_length" in
  ''|*[!0-9]*) echo "BLOCKED: max_target_path_length is missing or non-numeric: ${max_target_path_length:-<empty>}"; exit 264 ;;
esac
