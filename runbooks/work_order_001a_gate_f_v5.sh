...
# Work Order 001A — Approval Gate F v5
...
#
# Changes from v4:
#   1. The complete NUL-delimited manifest is parsed and validated in native
#      Windows Python before extraction begins.
#   2. The manifest and archive regular-file path sets must match exactly before
#      the first directory or file is written.
#   3. Each extracted regular file is SHA-256 verified inline while copied.
#   4. The extraction report records manifest_records and
#      manifest_verification=PASS, which Bash validates before continuing.
...
printf '%s\n' '=== WORK ORDER 001A GATE F GUARDED WINDOWS EXTRACTION V5 ==='
...
expected_manifest_records = int(expected_manifest_records)
destination = os.path.abspath(destination)

MAX_WINDOWS_TARGET_LENGTH = 259
...
def validate_windows_parts(parts, raw):
    for part in parts:
...
def parse_manifest(path):
    ...
    if not data or not data.endswith(b"\x00"):
        raise ValueError("manifest is empty or lacks a final NUL terminator")

    records = data[:-1].split(b"\x00")
    if any(record == b"" for record in records):
        raise ValueError("manifest contains an empty record")
    if len(records) != expected_manifest_records:
        raise ValueError(
            f"manifest records {len(records)} != {expected_manifest_records}"
        )
    ...
    for record in records:
        if len(record) < 69:
            raise ValueError(f"manifest record is too short: {record!r}")
        digest_bytes = record[:64]
        if DIGEST_RE.fullmatch(digest_bytes) is None:
            raise ValueError(f"manifest digest is not lowercase SHA-256: {record!r}")
        if record[64:65] != b" " or record[65:66] not in (b" ", b"*"):
            raise ValueError(f"manifest record has an invalid mode separator: {record!r}")

        filename_bytes = record[66:]
        try:
            filename = filename_bytes.decode("utf-8", "strict")
        except UnicodeDecodeError as exc:
            raise ValueError(f"manifest filename is not valid UTF-8: {filename_bytes!r}") from exc

        if not filename.startswith("./") or filename == "./":
            raise ValueError(f"manifest filename must start with './': {filename!r}")
        relative = filename[2:]
        if "\\" in relative:
            raise ValueError(f"manifest filename contains a backslash: {filename!r}")
        if relative.startswith("/") or windows_drive.match(relative) or ntpath.isabs(relative):
            raise ValueError(f"manifest filename is absolute or drive-qualified: {filename!r}")
        if any(ord(ch) < 32 or ord(ch) == 127 for ch in relative):
            raise ValueError(f"manifest filename contains a control character: {filename!r}")
        if "//" in relative:
            raise ValueError(f"manifest filename is non-canonical: {filename!r}")

        parts = relative.split("/")
        if any(part in ("", ".", "..") for part in parts):
            raise ValueError(f"manifest filename contains traversal or an empty component: {filename!r}")
        validate_windows_parts(parts, filename)

        normalized = posixpath.normpath(relative)
        if normalized != relative or normalized.startswith("../") or normalized == "..":
            raise ValueError(f"manifest filename escapes its root: {filename!r}")

        archive_path = "releases/" + relative
        if archive_path in digests:
            raise ValueError(f"duplicate manifest path: {filename!r}")

        windows_key = "/".join(part.casefold() for part in parts)
        if windows_key in seen_windows:
            raise ValueError(f"case-insensitive Windows manifest collision: {filename!r}")

        digests[archive_path] = digest_bytes.decode("ascii")
        seen_windows.add(windows_key)

    return digests


manifest_digests = parse_manifest(manifest)
seen_posix = set()
seen_windows = set()
prepared = []
archive_regular_paths = set()
regular = 0
directories = 0
regular_bytes = 0
root_directory_seen = False
max_target_path_length = 0


def prepare(member):
    global regular, directories, regular_bytes, root_directory_seen, max_target_path_length
    ...
    if any(ord(ch) < 32 or ord(ch) == 127 or ch in invalid_windows_chars for ch in raw):
    ...
    validate_windows_parts(parts, raw)
    ...
    if not is_dir:
        archive_regular_paths.add(normalized)
    prepared.append((member, target, parts, is_dir, normalized))
...
with tarfile.open(archive, mode="r:gz") as tf:
    ...
    if manifest_paths != archive_regular_paths:
        missing = sorted(archive_regular_paths - manifest_paths)
        extra = sorted(manifest_paths - archive_regular_paths)
        raise ValueError(
            "manifest/archive path-set mismatch: "
            f"missing_from_manifest={len(missing)} "
            f"extra_in_manifest={len(extra)} "
            f"missing_sample={missing[:3]!r} extra_sample={extra[:3]!r}"
        )

    # The archive and manifest are fully validated before the first write.
    directory_entries = sorted(
        (entry for entry in prepared if entry[3]),
        key=lambda entry: (len(entry[2]), entry[2]),
    )
    for member, target, parts, _, normalized in directory_entries:
        os.mkdir(target)

    actual_written = 0
    verified_paths = set()
    for member, target, parts, is_dir, normalized in prepared:
        if is_dir:
            continue
        ...
        expected_digest = manifest_digests[normalized]
        ...
        if actual_digest != expected_digest:
            raise ValueError(
                f"manifest digest mismatch for {normalized!r}: "
...
if verified_paths != set(manifest_digests):
    raise ValueError("verified path set no longer equals the manifest path set")
...
with open(report, "w", encoding="utf-8", newline="\n") as fh:
    ...
    fh.write(f"max_target_path_length={max_target_path_length}\n")
    fh.write(f"manifest_records={len(manifest_digests)}\n")
    fh.write("manifest_verification=PASS\n")
    fh.write("extraction=PASS\n")
PY

# Convert every Bash path passed to native Windows Python into Windows paths.
...
export DOMINION_MANIFEST="$MANIFEST_WIN"
...
export DOMINION_EXPECTED_MANIFEST_RECORDS="$EXPECTED_MANIFEST_RECORDS"
...
$arguments = @(
  ...
  [System.IO.Path]::GetFullPath($env:DOMINION_MANIFEST),
  ...
  $env:DOMINION_EXPECTED_MANIFEST_RECORDS
)
...
max_target_path_length=$(report_value max_target_path_length) || {
  echo 'BLOCKED: max_target_path_length report field is missing or duplicated'
  exit 264
}
case "$max_target_path_length" in
  ''|*[!0-9]*) echo "BLOCKED: max_target_path_length is non-numeric: ${max_target_path_length:-<empty>}"; exit 265 ;;
esac
[ "$max_target_path_length" -le 259 ] || {
  echo "BLOCKED: max_target_path_length $max_target_path_length exceeds Windows ceiling 259"
  exit 266
}

report_manifest_records=$(report_value manifest_records) || {
  echo 'BLOCKED: manifest_records report field is missing or duplicated'
  exit 267
}
case "$report_manifest_records" in
  ''|*[!0-9]*) echo "BLOCKED: manifest_records is non-numeric: ${report_manifest_records:-<empty>}"; exit 268 ;;
esac
[ "$report_manifest_records" = "$EXPECTED_MANIFEST_RECORDS" ] || {
  echo "BLOCKED: manifest_records $report_manifest_records != $EXPECTED_MANIFEST_RECORDS"
  exit 269
}

report_manifest_verification=$(report_value manifest_verification) || {
  echo 'BLOCKED: manifest_verification report field is missing or duplicated'
  exit 270
}
[ "$report_manifest_verification" = 'PASS' ] || {
  echo "BLOCKED: manifest_verification is not PASS: $report_manifest_verification"
  exit 271
}
...
source_manifest_records=$report_manifest_records
...
max_target_path_length=$max_target_path_length
manifest_verification=$report_manifest_verification
EOF
