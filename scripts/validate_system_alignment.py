...
COUNCIL_NODE_POLICY = ROOT / "governance" / "council_node_policy.json"
...
def validate_council_node_release_compliance() -> None:
    policy = load(COUNCIL_NODE_POLICY)
    compliance = load(COUNCIL_NODE_COMPLIANCE)

    require(policy.get("status") == "BUILT_NOT_DEPLOYED", "Council Node must remain predeployment in this release")
    require(policy.get("external_execution_authorized") is False, "Council Node external execution must remain disabled")
    bindings = policy.get("bindings", {})
    require(bindings.get("release_compliance") == "governance/council_node_release_compliance.json", "Council Node compliance evidence binding drift")

    require(compliance.get("schema_version") == 1, "Council Node release compliance schema_version must be 1")
    require(compliance.get("subsystem") == "council_node", "Council Node compliance subsystem mismatch")
    require(compliance.get("release_scope") == "code_only_predeployment", "Council Node release scope must remain code-only predeployment")
    require(compliance.get("runtime_status") == "BUILT_NOT_DEPLOYED", "Council Node runtime status drift")
    require(compliance.get("external_execution_authorized") is False, "Council Node compliance must not authorize external execution")
    require(compliance.get("compliance_certification_claimed") is False, "Council Node must not claim blanket legal compliance")

    privacy = compliance.get("privacy", {})
    require(privacy.get("regulated_personal_data_authorized") is False, "regulated personal data must not be authorized")
    require(privacy.get("secret_memory_write_rejected") is True, "secret memory writes must remain rejected")
    require(privacy.get("sensitive_semantic_memory_rejected") is True, "sensitive semantic memory must remain rejected")
    require(privacy.get("working_memory_ttl_seconds") == 3600, "working memory TTL evidence drift")
    require(set(privacy.get("external_forbidden_classifications", [])) == {"sensitive", "secret", "restricted"}, "external data classification boundary drift")
    require(privacy.get("external_models_enabled_by_default") is False, "external models must remain opt-in")
    require(privacy.get("default_bind_host") == "127.0.0.1", "Council Node must bind loopback by default")
    require(privacy.get("privacy_review_required_before_regulated_data_processing") is True, "privacy review must gate regulated data processing")

    contracts = compliance.get("contracts", {})
    require(contracts.get("external_provider_activation_authorized_in_this_release") is False, "external provider activation must remain outside this release")
    require(contracts.get("provider_terms_attestation_required_before_external_model_activation") is True, "provider terms attestation must gate external model activation")
    require(contracts.get("provider_contract_compliance_certified") is False, "provider contract compliance must not be falsely certified")
    require(contracts.get("external_models_are_opt_in") is True, "external models must remain opt-in")

    regulation = compliance.get("regulation", {})
    require(regulation.get("jurisdiction_assessment_required_before_regulated_data_processing") is True, "jurisdiction assessment must gate regulated data processing")
    require(regulation.get("regulated_data_processing_authorized_in_this_release") is False, "regulated data processing must remain outside this release")
    require(regulation.get("gdpr_compliance_certified") is False, "GDPR compliance must not be falsely certified")
    require(regulation.get("ccpa_compliance_certified") is False, "CCPA compliance must not be falsely certified")
    require(regulation.get("release_does_not_assert_jurisdiction_specific_compliance") is True, "release must not assert jurisdiction-specific compliance")

    gates = compliance.get("activation_gates", {})
    for key in (
        "production_deployment_requires_separate_release",
        "external_models_require_separate_governed_activation",
        "regulated_data_requires_legal_privacy_review",
        "provider_terms_attestation_required",
    ):
        require(gates.get(key) is True, f"Council Node activation gate missing: {key}")

    boundary = compliance.get("release_boundary", {})
    require(boundary.get("may_merge_code_after_five_council_approval") is True, "code merge boundary missing")
    require(boundary.get("may_deploy_production") is False, "this release must not authorize production deployment")
    require(boundary.get("may_enable_external_models") is False, "this release must not authorize external models")
    require(boundary.get("may_process_regulated_personal_data") is False, "this release must not authorize regulated personal data")


def validate() -> tuple[int, int]:
    validate_contract()
    validate_council_node_release_compliance()
...
def main() -> int:
    ...
    print("COUNCIL_NODE_RELEASE_COMPLIANCE_BOUNDARY=PASS")
    return 0
